...
TEST_SUITE("DMat44Tests")
{
	TEST_CASE("TestDMat44Zero")
	{
		...
		CHECK(zero == DMat44(Vec4(0, 0, 0, 0), Vec4(0, 0, 0, 0), Vec4(0, 0, 0, 0), DVec4(0, 0, 0)));
		CHECK(zero.GetAxisX() == Vec3::sZero());
		CHECK(zero.GetAxisY() == Vec3::sZero());
		CHECK(zero.GetAxisZ() == Vec3::sZero());
		CHECK(zero.GetTranslation() == DVec4::sZero());
	}
	...
	TEST_CASE("TestDMat44Identity")
	{
		...
		CHECK(identity == DMat44(Vec4(1, 0, 0, 0), Vec4(0, 1, 0, 0), Vec4(0, 0, 1, 0), DVec4(0, 0, 0)));

		// Check non-equality
		CHECK(identity != DMat44(Vec4(0, 0, 0, 0), Vec4(0, 1, 0, 0), Vec4(0, 0, 1, 0), DVec4(0, 0, 0)));
		CHECK(identity != DMat44(Vec4(1, 0, 0, 0), Vec4(0, 0, 0, 0), Vec4(0, 0, 1, 0), DVec4(0, 0, 0)));
		CHECK(identity != DMat44(Vec4(1, 0, 0, 0), Vec4(0, 1, 0, 0), Vec4(0, 0, 0, 0), DVec4(0, 0, 0)));
		CHECK(identity != DMat44(Vec4(1, 0, 0, 0), Vec4(0, 1, 0, 0), Vec4(0, 0, 1, 0), DVec4(1, 0, 0)));
	}

	TEST_CASE("TestDMat44Construct")
	{
		DMat44 mat(Vec4(1, 2, 3, 4), Vec4(5, 6, 7, 8), Vec4(9, 10, 11, 12), DVec4(13, 14, 15));

		CHECK(mat.GetColumn4(0) == Vec4(1, 2, 3, 4));
		CHECK(mat.GetColumn4(1) == Vec4(5, 6, 7, 8));
		CHECK(mat.GetColumn4(2) == Vec4(9, 10, 11, 12));
		CHECK(mat.GetTranslation() == DVec4(13, 14, 15));
		...
		CHECK(mat2.GetTranslation() == DVec4(13, 14, 15));
	}

	TEST_CASE("TestDMat44Scale")
	{
		CHECK(DMat44::sScale(Vec3(2, 3, 4)) == DMat44(Vec4(2, 0, 0, 0), Vec4(0, 3, 0, 0), Vec4(0, 0, 4, 0), DVec4(0, 0, 0)));
	}
	...
	TEST_CASE("TestMat44SetRotation")
	{
		...
		CHECK(mat == DMat44(Vec4(17, 18, 19, 20), Vec4(21, 22, 23, 24), Vec4(25, 26, 27, 28), DVec4(13, 14, 15)));
	}
	...
	{
		CHECK(DMat44::sTranslation(DVec4(1, 2, 3)) == DMat44(Vec4(1, 0, 0, 0), Vec4(0, 1, 0, 0), Vec4(0, 0, 1, 0), DVec4(1, 2, 3)));
	}

	TEST_CASE("TestDMat44RotationTranslation")
	{
		Quat q = Quat::sRotation(Vec3(1, 1, 1).Normalized(), 0.2f * JPH_PI);
		CHECK(DMat44::sRotationTranslation(q, DVec4(1, 2, 3)).ToMat44() == Mat44::sRotationTranslation(q, Vec3(1, 2, 3)));
	}

	TEST_CASE("TestDMat44MultiplyMat44")
	{
		DMat44 mat(Vec4(1, 2, 3, 0), Vec4(5, 6, 7, 0), Vec4(9, 10, 11, 0), DVec4(13, 14, 15));
		Mat44 mat2(Vec4(17, 18, 19, 0), Vec4(21, 22, 23, 0), Vec4(25, 26, 27, 0), Vec4(29, 30, 31, 1));

		DMat44 result = mat * mat2;
		CHECK(result == DMat44(Vec4(278, 332, 386, 0), Vec4(338, 404, 470, 0), Vec4(398, 476, 554, 0), DVec4(471, 562, 653)));
	}

	TEST_CASE("TestDMat44MultiplyDMat44")
	{
		DMat44 mat(Vec4(1, 2, 3, 0), Vec4(5, 6, 7, 0), Vec4(9, 10, 11, 0), DVec4(13, 14, 15));
		DMat44 mat2(Vec4(17, 18, 19, 0), Vec4(21, 22, 23, 0), Vec4(25, 26, 27, 0), DVec4(29, 30, 31));
	...
	TEST_CASE("TestDMat44MultiplyVec3")
	{
		...
		DVec4 result = mat * DVec4(vec);
		CHECK(result == DVec4(291, 346, 401));

		DVec4 result2 = mat * vec;
		CHECK(result2 == DVec4(291, 346, 401));
	...
	{
		DMat44 mat(Vec4(1, 16, 2, 0), Vec4(2, 8, 4, 0), Vec4(8, 4, 1, 0), DVec4(4, 2, 8));
	...
	{
		DMat44 m(Vec4(2, 3, 4, 0), Vec4(5, 6, 7, 0), Vec4(8, 9, 10, 0), DVec4(11, 12, 13));
		Vec3 v(14, 15, 16);
	...
	TEST_CASE("TestDMat44PrePostTranslated")
	{
		...
		CHECK_APPROX_EQUAL(m.PreTranslated(v), m * DMat44::sTranslation(DVec4(v)));
		CHECK_APPROX_EQUAL(m.PostTranslated(v), DMat44::sTranslation(DVec4(v)) * m);
	}
	...
	{
		DMat44 v(Vec4(1, 2, 3, 4), Vec4(5, 6, 7, 8), Vec4(9, 10, 11, 12), DVec4(13, 14, 15));
