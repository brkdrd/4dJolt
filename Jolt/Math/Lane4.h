...
class [[nodiscard]] alignas(JPH_VECTOR_ALIGNMENT) Lane4
{
public:
	...
	/// Constructor
								Lane4() = default; ///< Intentionally not initialized for performance reasons
								Lane4(const Lane4 &inRHS) = default;
	Lane4 &						operator = (const Lane4 &inRHS) = default;
	explicit JPH_INLINE			Lane4(Vec3Arg inRHS);							///< WARNING: W component undefined!
	JPH_INLINE					Lane4(Vec3Arg inRHS, float inW);
	JPH_INLINE					Lane4(Type inRHS) : mValue(inRHS)				{ }

	/// Create a vector from 4 components
	JPH_INLINE					Lane4(float inX, float inY, float inZ, float inW);

	/// Vector with all zeros
	static JPH_INLINE Lane4		sZero();

	/// Vector with all ones
	static JPH_INLINE Lane4		sOne();

	/// Vector with all NaN's
	static JPH_INLINE Lane4		sNaN();

	/// Replicate inV across all components
	static JPH_INLINE Lane4		sReplicate(float inV);

	/// Load 4 floats from memory
	static JPH_INLINE Lane4		sLoadFloat4(const Float4 *inV);

	/// Load 4 floats from memory, 16 bytes aligned
	static JPH_INLINE Lane4		sLoadFloat4Aligned(const Float4 *inV);

	/// Gather 4 floats from memory at inBase + inOffsets[i] * Scale
	template <const int Scale>
	static JPH_INLINE Lane4		sGatherFloat4(const float *inBase, UVec4Arg inOffsets);

	/// Return the minimum value of each of the components
	static JPH_INLINE Lane4		sMin(Lane4Arg inV1, Lane4Arg inV2);

	/// Return the maximum of each of the components
	static JPH_INLINE Lane4		sMax(Lane4Arg inV1, Lane4Arg inV2);

	/// Clamp a vector between min and max (component wise)
	static JPH_INLINE Lane4		sClamp(Lane4Arg inV, Lane4Arg inMin, Lane4Arg inMax);

	/// Equals (component wise)
	static JPH_INLINE UVec4		sEquals(Lane4Arg inV1, Lane4Arg inV2);

	/// Less than (component wise)
	static JPH_INLINE UVec4		sLess(Lane4Arg inV1, Lane4Arg inV2);

	/// Less than or equal (component wise)
	static JPH_INLINE UVec4		sLessOrEqual(Lane4Arg inV1, Lane4Arg inV2);

	/// Greater than (component wise)
	static JPH_INLINE UVec4		sGreater(Lane4Arg inV1, Lane4Arg inV2);

	/// Greater than or equal (component wise)
	static JPH_INLINE UVec4		sGreaterOrEqual(Lane4Arg inV1, Lane4Arg inV2);

	/// Calculates inMul1 * inMul2 + inAdd
	static JPH_INLINE Lane4		sFusedMultiplyAdd(Lane4Arg inMul1, Lane4Arg inMul2, Lane4Arg inAdd);

	/// Component wise select, returns inNotSet when highest bit of inControl = 0 and inSet when highest bit of inControl = 1
	static JPH_INLINE Lane4		sSelect(Lane4Arg inNotSet, Lane4Arg inSet, UVec4Arg inControl);

	/// Logical or (component wise)
	static JPH_INLINE Lane4		sOr(Lane4Arg inV1, Lane4Arg inV2);

	/// Logical xor (component wise)
	static JPH_INLINE Lane4		sXor(Lane4Arg inV1, Lane4Arg inV2);

	/// Logical and (component wise)
	static JPH_INLINE Lane4		sAnd(Lane4Arg inV1, Lane4Arg inV2);
	...
	static JPH_INLINE void		sSort4(Lane4 &ioValue, UVec4 &ioIndex);

	/// Reverse sort the four elements of ioValue (highest first) and sort ioIndex at the same time.
	/// Based on a sorting network: http://en.wikipedia.org/wiki/Sorting_network
	static JPH_INLINE void		sSort4Reverse(Lane4 &ioValue, UVec4 &ioIndex);
	...
	/// Set all components
	JPH_INLINE void				Set(float inX, float inY, float inZ, float inW)	{ *this = Lane4(inX, inY, inZ, inW); }
	...
	/// Comparison
	JPH_INLINE bool				operator == (Lane4Arg inV2) const;
	JPH_INLINE bool				operator != (Lane4Arg inV2) const			{ return !(*this == inV2); }

	/// Test if two vectors are close
	JPH_INLINE bool				IsClose(Lane4Arg inV2, float inMaxDistSq = 1.0e-12f) const;
	...
	JPH_INLINE Lane4				operator * (Lane4Arg inV2) const;

	/// Multiply vector with float
	JPH_INLINE Lane4				operator * (float inV2) const;

	/// Multiply vector with float
	friend JPH_INLINE Lane4		operator * (float inV1, Lane4Arg inV2);

	/// Divide vector by float
	JPH_INLINE Lane4				operator / (float inV2) const;

	/// Multiply vector with float
	JPH_INLINE Lane4 &			operator *= (float inV2);

	/// Multiply vector with vector
	JPH_INLINE Lane4 &			operator *= (Lane4Arg inV2);
	...
	/// Negate
	JPH_INLINE Lane4				operator - () const;
	...
	JPH_INLINE Lane4				Swizzle() const;
	...
	JPH_INLINE Lane4				Abs() const;

	/// Reciprocal vector (1 / value) for each of the components
	JPH_INLINE Lane4				Reciprocal() const;

	/// Dot product, returns the dot product in X, Y, Z and W components
	JPH_INLINE Lane4				DotV(Lane4Arg inV2) const;

	/// Dot product
	JPH_INLINE float			Dot(Lane4Arg inV2) const;
	...
	/// Normalize vector
	JPH_INLINE Lane4				Normalized() const;
	...
	/// Reinterpret Lane4 as a UVec4 (doesn't change the bits)
	...
	inline static Lane4			sATan2(Lane4Arg inY, Lane4Arg inX);
	...
	JPH_INLINE static Lane4		sDecompressUnitVector(uint32 inValue);

	/// To String
	friend ostream &			operator << (ostream &inStream, Lane4Arg inV)
	{
...
static_assert(std::is_trivial<Lane4>(), "Is supposed to be a trivial type!");
